...
success_msg("Richtig. Am neunstündigen Freitag wurden 5110€ Umsatz pro Stunde erwirtschaftet. Dies ist als gerade so profitabel anzusehen! Ein dickes Lob an Sie, Sie lernen schnell - nur noch 2 Einheiten (4/6 abgeschlossen)")
```
...
I. Vektoren in einer Matrix (my.matrix) zusammenführen: 
...
success_msg("Richtig korrigiert. Der finale Bericht geht nun an die Geschäftsleitung.")
```
...
`@feedback`
- Richtig - in Matrizen können nur Daten **gleichen Datentyps** gespeichert werden. In Data Frames können Elemente unterschiedlichen Typs gleicher Zeilenlänge gespeichert werden. Fast fertig, auf geht es zur letzten Einheit! **(5/6 Einheiten abgeschlossen)**
...
Der **letzte** Aufgabenteil für Sie: Nach den Vektoren und Martrizen, kommen wir nun zur Spezialform von Listen, den **Data Frames**. 
Data Frames sind Listen mit Vektoren gleicher Länge, die bei der Datenanalyse in R sehr häufig Anwendung finden, da in einem Data Frame unterschiedliche Datentypen gespeichert werden können  (Grolemund und Wickham 2015).
...
success_msg("Ja, genau. Die Funktionen str() und summary() sind sehr hilfreich und verschaffen Ihnen einen kompakten Überblick über den Datensatz. Sie haben Ihren Arbeitstag so gut wie geschafft und alle Aufgaben mit höchster Genauigkeit bearbeitet. Herr Müller ist mit Ihnen sehr zufrieden und hat noch eine kleine Aufagbe, bevor es dann in den Feierabend geht (5.5/6)")
```
